...
-- Expect a keyword, but if we get anything else it's a fatal errorexport
...
||| BNF:
||| plainBinder := name ':' typeExprexport
...
||| BNF:
||| basicMultiBinder := name (, name)* ':' typeExprexport
...
-- a data declaration can have a visibility and an optional totality (#1404)export
